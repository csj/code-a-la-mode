...
class Player(val isLeftTeam: Boolean = true) : AbstractPlayer() {
  ...
  fun use(cell: Cell) {
    ...
    if (cell.distanceTo(location) > REACH_DISTANCE) throw Exception("Cannot use: too far!")
    equipment.use(this)
  }

  fun drop(cell: Cell) {
    val item = heldItem ?: throw Exception("Cannot drop: not holding anything!")
    if (cell.distanceTo(location) > REACH_DISTANCE) throw Exception("Cannot drop: too far!")
  ...
  fun take(cell: Cell) {
    ...
    if (cell.distanceTo(location) > REACH_DISTANCE) throw Exception("Cannot take: too far!")
  ...
  fun moveTo(cell: Cell) {
    if (location.distanceTo(cell) > WALK_DISTANCE) throw Exception("Cannot move: too far!")
